...
const MemberSidebar = () => {
	...
	useEffect(() => {
		...
		const serverMembers = all.filter(
			(member: MemberType) => member.role === "member"
		)
		setMembers(serverMembers)
		const serverOwners = all.filter(
			(member: MemberType) => member.role === "owner"
		)
		setOwners(serverOwners)
		const serverModerators = all.filter(
			(member: MemberType) => member.role === "admin"
		)
